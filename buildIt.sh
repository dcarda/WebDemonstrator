...
#===  FUNCTION   ================================================
#         Name:  build_EarArtifact_noTests()
#  --------------------------------------------------------------
...
#      Returns:  None
#================================================================
function build_EarArtifact_noTests() {
...
} # ---  End Function build_EarArtifact_noTests()
...
function build_SiteReports() {
    ...
    # =======================================================================
    #  Build everything
    #  mvn   -P genReports clean package site javancss:report clover:clover
    mvn                \
      -P genReports    \
      clean            \
      package          \
...
function delete_Artifacts() {
    ...
    # Delete EAR
    rm --force --recursive ./ear/target/
    if [ $? -ne 0 ]; then
        echo
        echo "Failure: Cannot Delete: ./ear/target/"
        terminationAbnormal
    fi

    # Delete WAR
    rm --force --recursive ./web/target/
    if [ $? -ne 0 ]; then
        echo
        echo "Failure: Cannot Delete: ./web/target/"
        terminationAbnormal
    fi

    # Delete EJB
    rm --force --recursive ./ejb/target/
    if [ $? -ne 0 ]; then
        echo
        echo "Failure: Cannot Delete: ./ejb/target/"
        terminationAbnormal
    fi
